...
/*
final_state([[un,deux,trois,quatre],
			[cinq,six,sept,huit],
			[neuf,dix,onze,douze],
			[treize,quatorze,quinze,vide]]).
*/	 
   %********************
   % AFFICHAGE D'UN ETAT
   %********************
...
heuristique(U,H) :-
   ...
   % même piece.
	...
	diff_ligne([],[], 0).
	diff_ligne([vide|Reste1],[_|Reste2],Diff) :- diff_ligne(Reste1,Reste2, Diff).
	diff_ligne([A|Reste1], [A|Reste2], Diff) :- A\=vide, diff_ligne(Reste1,Reste2, Diff).
	diff_ligne([A|Reste1], [B|Reste2], Diff) :- 
		A \= vide, 
		A \= B,
		diff_ligne(Reste1,Reste2, SousDiff),
		Diff is SousDiff + 1.
		
		

	diff_matrice([],[], 0).
	diff_matrice([Ligne1|Reste1],[Ligne2|Reste2], Diff) :- 
		diff_ligne(Ligne1,Ligne2,A),
		diff_matrice(Reste1,Reste2,B),
		Diff is A+B.

	   

    heuristique1(U, H) :- final_state(Final), diff_matrice(U,Final,H).
   ...
   % entre sa position courante et sa positon dans l'etat final
	...
	distance_manhattan([],[],_,0).
	distance_manhattan(_,_,vide,0).
	distance_manhattan(MatDepart,MatArrivee, P, Dist) :-				
				coordonnees([L1,C1],MatDepart,P),
				P \= vide,
				coordonnees([L2,C2],MatArrivee,P),
				Dist is abs(L2-L1) + abs(C2-C1).

    heuristique2(U, H) :- 
		final_state(Final), 
		findall(Distance,distance_manhattan(U,Final,_,Distance),Distances),		
